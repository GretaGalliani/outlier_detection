...
# Import dataset BTC-USD
btc = read.csv('BTC-USD.csv')
date = as.Date.character(btc$Date)
btc$Date = NULL
btc$LogReturn = rep(0, dim(btc)[1])

for (i in 2:dim(btc)[1]){
  btc$LogReturn[i] = log(btc$Adj.Close[i]/btc$Adj.Close[i-1])*100
}

btc$Adj.Close = NULL

x = seq(min(btc$LogReturn), max(btc$LogReturn), by=0.1)
y = dnorm(x, mean=mean(btc$LogReturn), sd=sd(btc$LogReturn))
hist(btc$LogReturn, freq=FALSE, xlim=range(x), ylim=range(y))
points(x, y, type='l')


ts_plot(data.frame(date = date, LogReturn=btc$LogReturn))

data = as.matrix(scale(btc))
n = dim(data)[1]
d = dim(data)[2]
...
plot(btc$LogReturn, col=col_min_bind, pch = bind_pch, main = "Partition minimizing Binder Loss")
