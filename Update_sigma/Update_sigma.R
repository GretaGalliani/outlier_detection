...
update_sigma <- function(m1, m1_bar, k, sigma_old, theta, freq, sd = 2) { 
  ...
  print(y)
  aprob <- calcolo_alpha_sigma(sigma_old, y, k, m1_bar, theta, freq)
  print(aprob)
...
# DA TESTARE
calcolo_alpha_sigma <- function(x, y, k, m1_bar, theta, freq) {
  pi_y <- dens_sigma(y, k, m1_bar, theta, freq)
  pi_x <- dens_sigma(x, k, m1_bar, theta, freq)
  rapp <- pi_y/pi_x
  return (min(1, rapp))
}
